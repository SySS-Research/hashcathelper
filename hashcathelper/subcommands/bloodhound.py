...
def domain_filepath_pair(arg):
    if ":" not in arg:
        log.critical(
            "Argument contains no colon: %s. (Note the new argument format.)" % arg
        )
        exit(1)
...
def add_cracked_attribute(args):
    ...
    for domain, infile in args.domain_infile:
        ...
        if "full_creds" not in data:
            log.critical(
                "No information about cracked users found in report file (did you use `--degree-of-detail 4`?)"
            )
            exit(1)

        users.extend(
            ("%s@%s" % (user, domain)).upper() for user in data["full_creds"].keys()
        )
...
def add_samepassword_relationships(args):
    ...
    for domain, infile in args.domain_infile:
        ...
        if "details" not in data or "clusters" not in data["details"]:
            log.critical(
                "No information about clusters found in report file (did you use `--degree-of-detail 3`?)"
            )
            exit(1)

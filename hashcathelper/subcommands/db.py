...
been filtered."""
    ...
    result["submitter_email"] = ask_question(
        "Please provide your e-mail address in case questions come up" " (optional)",
    )
    ...
    try:
        hashcat_version = (
            subprocess.check_output([config.hashcat_bin, "-V"]).decode().strip()
        )
        ...
        for r in s.query(Report).order_by(Report.id.asc()).all():
            if args.raw:
                ...
                data.append(
                    [
                        r.id,
                        r.submission_date.replace(microsecond=0),
                        r.submitter_email,
                        r.accounts,
                    ]
                )
...
def stats(args):
    ...
    if args.format in ["text", "html"]:
        ...
        out = "The database holds information about %d accounts in %d entries.\n" % (
            total_accounts,
            total_entries,
        )
...
def get_stats(entry, all_entries):
    ...
    all_entries = [
        orm_to_dict(e, relative_quantities, absolute_quantities) for e in all_entries
    ]
